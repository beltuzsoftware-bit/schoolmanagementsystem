...
zip_path = r"d:\kummi-school-system\KuMMi_Portable_Backup_2026-06-02_1241.zip"
target_snapshot = "data.json.before_restore_2026-05-20.json"
workspace_dir = r"d:\kummi-school-system"
backup_folder = r"d:\kummi-school-system\KuMMi_Portable_Backup_2026-06-02_1241"
active_db_path = r"d:\kummi-school-system\data.json"
safety_backup_path = r"d:\kummi-school-system\data.json.before_june2_restore.json"

# Step 1: Copy latest source code and files
print("Step 1: Restoring source code and files from June 2 backup directory...")

ignore_dirs = {
    "node_modules", ".next", ".git", ".gemini", "backups", "brain", 
    "restore_temp_0048", "restore_temp_0507", "restore_temp_1402", 
    "temp_backup_inspect", "KuMMi_Portable_Backup_2026-06-02_1241"
}
...
def copy_recursive(src, dst):
    ...
    for item in os.listdir(src):
        ...
        if os.path.isdir(src_path):
            ...
            # Skip any zip files to avoid huge copies
...
if os.path.exists(backup_folder):
    print(f" -> Found extracted backup folder: {backup_folder}")
    copy_recursive(backup_folder, workspace_dir)
    print(" -> Code and configuration files successfully copied!")
else:
    print(f" ❌ Error: June 2 backup directory '{backup_folder}' not found! Cannot copy files.")
    sys.exit(1)

# Step 2: Safety backup of active database
print("\nStep 2: Creating safety backup of active data.json...")
...
# Step 3: Extract database safety snapshot
print(f"\nStep 3: Extracting snapshot '{target_snapshot}' from {zip_path}...")
...
# Step 4: Patch missing school and users
print("\nStep 4: Patching missing 4th School definition (s_1778721678173)...")
schools = db.get("schools", [])

sample_school = schools[0] # Use JSI as base template
tuscany_school = {
    "id": "s_1778721678173",
...
# Patch complete set of 10 users to import
target_users = [
...
try:
    ...
    print("🎉 SUCCESS! Prisma migration completed successfully.")
except subprocess.CalledProcessError as e:
    print("❌ ERROR: Prisma migration failed!")
    print("Error output:\n", e.stderr)
    sys.exit(1)

print("\nStep 7: Repairing User schoolId mappings in live database...")
try:
    result = subprocess.run(["node", "scratch/fix_live_users.cjs"], capture_output=True, text=True, check=True)
    print(" -> Mappings output:\n", result.stdout)
    print("🎉 SUCCESS! Live user mappings successfully repaired in Supabase.")
except subprocess.CalledProcessError as e:
    print("❌ ERROR: User mapping repair failed!")
    print("Error output:\n", e.stderr)
    sys.exit(1)
