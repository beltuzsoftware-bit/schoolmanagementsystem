...
export default function StudentFeeDetailsV2({ 
    ...
    const selectedMonthsData = useMemo(() => {
        return selectedMonthIndices.map(mIdx => {
                ...
                if (transportSettled < adjustedMonthlyFee) {
                    ...
                    if (fineAmt > 0) {
                        ...
                        if (isEffective) {
                            ...
                            const safeStart = (schoolDetails as any)?.sessionStartMonth || 1;
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                setActiveReceiptTxns(result.transactions as any);
                setIsReceiptModalOpen(true);
        ...
        try {
            ...
            reader.onload = async () => {
                ...
                if (result.success) {
                    ...
                    toast.error(result.error || 'Failed to update student photo.');
                }
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-[minmax(0,1fr)_340px] gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700 w-full">
            ...
            <div className="space-y-6">
                <Card className="border-none shadow-xl bg-white rounded-[32px] overflow-hidden sticky top-8">
                    <div className="bg-[#D0FAE5] p-5 text-slate-900 rounded-t-[32px] border-b border-emerald-100">
                        <div className="flex justify-between items-center mb-4">
                             ...
                             <div className="px-2 py-0.5 bg-emerald-600/10 rounded-full text-[9px] font-bold text-emerald-700 border border-emerald-200">
                    ...
                    <CardContent className="p-5 space-y-4 bg-white">
                        ...
                        <div className="grid grid-cols-[1fr_120px] gap-3">
                            ...
                            <div className="space-y-1.5 flex flex-col justify-end">
                                ...
                                <div className="flex p-1 bg-slate-100 rounded-2xl h-11 border border-slate-200/60 shadow-inner mt-1.5">
                                    <button 
                                        type="button"
                                        className={`flex-1 rounded-xl text-[9px] font-black transition-all ${receiptMode === 'auto' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                                    ...
                                    <button 
                                        type="button"
                                        className={`flex-1 rounded-xl text-[9px] font-black transition-all ${receiptMode === 'custom' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                        ...
                        {/* Presets Selector (Segmented Tabs style) */}
                        <div className="space-y-1.5 mt-2">
                            <Label className="text-[9px] font-black text-slate-400 uppercase tracking-[0.15em]">Quick Presets</Label>
                            <div className="grid grid-cols-3 gap-1 bg-slate-100 p-1 rounded-2xl h-11 border border-slate-200/60 shadow-inner">
                                <button 
                                    type="button"
                        ...
                        {/* Payment Mode (Full Width Select) */}
                        <div className="space-y-1.5 mt-3">
                            <Label className="text-[9px] font-black text-slate-400 uppercase tracking-[0.15em]">Payment Method</Label>
                            <Select value={paymentMode || "none"} onValueChange={(val) => setPaymentMode(val === 'none' ? '' : val)}>
                                ...
                                <SelectContent className="rounded-2xl border-slate-150">
                                    <SelectItem value="none" className="font-medium text-slate-400 italic text-xs rounded-xl">Select Payment Method</SelectItem>
